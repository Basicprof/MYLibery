...
public class ФазаПотоков {
    ...
    private static void start(List<Character> characters) throws InterruptedException {
        ...
        for (final Character character : characters) {
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    phaser.arriveAndAwaitAdvance();// Ждем все потоки пока они не выполнят это действие
                    if (!isEveryoneReady) {
        ...
        }phaser.arriveAndDeregister();// Отпускаем главный поток так как он не выполняет те тействия которые выполняют все потоки
    }
}
