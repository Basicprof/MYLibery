...
public class DrugsController {
    ...
    public synchronized void sell(Drug drug, int count) {
        String name = Thread.currentThread().getName();// запоминаем текущую нить
        if (!allDrugs.containsKey(drug)) { //Проверяем наличие лекарства к ключем drug
            System.out.println("Нет в наличии");
        }
        Integer currentCount = allDrugs.get(drug);// Возвращаем значение по ключу
        if (currentCount < count) {
            System.out.println(String.format("%s хочет %s %d шт. " +
                    "В наличии - %d", name, drug.getName(), count, currentCount));
        } else {
            allDrugs.put(drug, (currentCount - count));
            System.out.println(String.format("%s купил(а) " +
                    "%s %d шт. Осталось - %d", name, drug.getName(),
